...
How the `sbol::upgrade` module and the `sbol upgrade` subcommand are
gated in CI. For the conversion model itself — what gets preserved
in the backport namespace, how SequenceAnnotation collapses work, how
MapsTo decomposes — see [`conversion.md`](conversion.md).
...
`Document::upgrade_from_sbol2` and diffs the deterministically sorted
...
- **libSBOLj3 cross-impl** ([`crates/sbol/tests/cross_impl.rs`](../crates/sbol/tests/cross_impl.rs))
  parses our SBOL 3 output through libSBOLj3 and asserts triple-set
  equivalence with the Java reference.
- **pySBOL3 cross-impl** ([`crates/sbol/tests/cross_impl_pysbol3.rs`](../crates/sbol/tests/cross_impl_pysbol3.rs))
...
2. Call [`Document::upgrade_from_sbol2`](../crates/sbol/src/document.rs).
3. Sort, dedup, and N-Triples-canonicalize the resulting graph using
   [`sbol::upgrade::canonical_nt_line`](../crates/sbol/src/upgrade/mod.rs).
...
[`crates/sbol/examples/synbiohub_upgrade.rs`](../crates/sbol/examples/synbiohub_upgrade.rs)
fetches a real iGEM part as SBOL 2 from SynBioHub, runs
[`Document::upgrade_from_sbol2`], validates, and prints a structural
summary.
