...
| You have… | Run… | Get… |
|---|---|---|
| `*.xml` / `*.ttl` / `*.jsonld` from SynBioHub, iGEM Registry, JBEI ICE | `sbol upgrade` or [`Document::upgrade_from_sbol2`] | SBOL 3 RDF |
...
| SBOL 3 to publish to a tool that only consumes SBOL 2 | `sbol downgrade` or [`Document::downgrade_to_sbol2`] | SBOL 2 RDF |
...
```rust,no_run
use sbol::{Document, SbolIdentified};

let (document, report) = Document::upgrade_from_sbol2_path("BBa_F2620.xml")?;
...
This works because the upgrade preserves SBOL 2 identities, types, versions, and unmapped predicates under a `http://sboltools.org/backport#` namespace. The downgrade reads those triples to rebuild the original SBOL 2 shape. The [empirical round-trip report](sbol3-round-trip-report.md) verifies this against the SynBioHub, iGEM, and SBOLTestSuite fixtures.
...
- **API reference** — module docs for [`sbol::upgrade`](https://docs.rs/sbol/latest/sbol/upgrade) and [`sbol::downgrade`](https://docs.rs/sbol/latest/sbol/downgrade); GenBank and FASTA importers under [`sbol_genbank`](https://docs.rs/sbol-genbank) and [`sbol_fasta`](https://docs.rs/sbol-fasta).
- **Per-fixture round-trip results** — [`sbol3-round-trip-report.md`](sbol3-round-trip-report.md), regenerated by `cargo run -p sbol --bin generate-round-trip-report` against the committed SBOL 2 fixture corpus.
...
[`Document::upgrade_from_sbol2`]: https://docs.rs/sbol/latest/sbol/struct.Document.html#method.upgrade_from_sbol2
[`Document::downgrade_to_sbol2`]: https://docs.rs/sbol/latest/sbol/struct.Document.html#method.downgrade_to_sbol2
