...
fn implemented_validation_rules_have_regression_cases() {
    ...
    for status in statuses {
        ...
        if matches!(
            status.status,
            sbol3::RuleStatus::Unimplemented | sbol3::RuleStatus::MachineUncheckable
        ) {
            continue;
        }
        assert!(
...
fn machine_checkable_rules_have_positive_cases() {
    let positives: BTreeSet<&str> = all_positive_cases().iter().map(|case| case.rule).collect();
    let mut missing: Vec<&str> = Vec::new();

    for status in sbol3::validation_rule_statuses() {
        // Only machine-checkable rules need a selectivity (positive)
        // case: a locally valid instance of the construct the rule
        // governs that must NOT report the rule.
        if matches!(
            status.status,
            sbol3::RuleStatus::Unimplemented | sbol3::RuleStatus::MachineUncheckable
        ) {
        ...
        if !positives.contains(status.rule) {
            missing.push(status.rule);
        }
    }
...
/// Renders the per-rule negative + positive coverage matrix over every
/// machine-checkable rule. Firing of negatives is asserted by
/// `validation_rule_regression_cases_report_expected_rule_ids`;
/// non-firing of positives by `positive_rule_cases_do_not_report_their_rule`.
/// This renders the audit artifact committed at `docs/negative-coverage.md`.
fn render_coverage_matrix() -> String {
    ...
    let machine_checkable: Vec<_> = sbol3::validation_rule_statuses()
        .iter()
        .filter(|status| {
            !matches!(
                status.status,
    ...
    out.push_str("# SBOL 3.1.0 negative + positive coverage matrix\n\n");
    out.push_str(
        "Generated by `SBOL_UPDATE_NEGATIVE_COVERAGE=1 cargo test -p sbol3 --test \
         validation_rules negative_coverage_matrix_matches_committed_file`. CI runs the \
         same test without the variable as a freshness gate.\n\n",
    );
    writeln!(
        out,
        "Every one of the {} machine-checkable rules has at least one negative fixture (a \
         violating document asserted to report the rule at its catalog severity) and at \
         least one positive fixture (a valid instance asserted not to report it). \
         Resolver- and topology-gated rules carry their negative under a dedicated \
         external/option test rather than the offline `cases()` corpus.\n",
        machine_checkable.len()
    )
    .unwrap();

    out.push_str("| Rule | Section | Status | Normative | Blocker | Negatives | Positives | Negative source |\n");
    out.push_str("|------|---------|--------|-----------|---------|-----------|-----------|-----------------|\n");
    ...
    for status in &machine_checkable {
        ...
        let source = if neg > 0 {
            "`cases()`"
        } else if rule_cases::EXTERNAL_POLICY_RULES.contains(&status.rule) {
            "`external_validation.rs` (resolver mode)"
        } else if rule_cases::OPTION_POLICY_RULES.contains(&status.rule) {
            "topology-completeness test"
        } else {
            "MISSING"
        };
        let blocker = match status.blocker {
            Some(blocker) => format!("{blocker:?}"),
            None => "—".to_string(),
        };
        writeln!(
            out,
...
fn negative_coverage_matrix_matches_committed_file() {
    ...
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("docs")
        .join("negative-coverage.md");

    assert!(
        !rendered.contains("MISSING"),
        "a machine-checkable rule has neither a negative case nor a resolver/topology exemption"
    );

    if std::env::var_os("SBOL_UPDATE_NEGATIVE_COVERAGE").is_some() {
        std::fs::write(&path, &rendered).expect("write negative-coverage.md");
        return;
    }
    ...
    assert_eq!(
        committed, rendered,
        "docs/negative-coverage.md is stale. Run `SBOL_UPDATE_NEGATIVE_COVERAGE=1 cargo \
         test -p sbol --test validation_rules negative_coverage_matrix_matches_committed_file`."
    );
}
...
fn validation_rule_regression_cases_report_expected_rule_ids() {
    for case in all_rule_cases() {
        ...
        let issues = match case.severity {
            sbol3::Severity::Error => report.errors().collect::<Vec<_>>(),
            sbol3::Severity::Warning => report.warnings().collect::<Vec<_>>(),
...
fn every_rule_appears_in_exactly_one_coverage_bucket() {
    ...
    let catalog_rules: BTreeSet<&str> = sbol3::validation_rule_statuses()
        .iter()
...
fn every_appendix_b_triangle_rule_is_marked_machine_uncheckable() {
    ...
    let actual: BTreeSet<&str> = sbol3::validation_rule_statuses()
        .iter()
...
fn override_unknown_rule_id_is_rejected() {
    let err = sbol3::ValidationOptions::default()
        .deny("sbol3-99999")
...
fn external_mode_provided_promotes_resolver_blocked_rules_to_fully_applied() {
    ...
    let resolver_partial: Vec<_> = report
        ...
        .partially_applied
        .iter()
        .filter(|partial| partial.blocker == sbol3::Blocker::Resolver)
    ...
    let context = sbol3::ValidationContext::new()
        .with_external_mode(sbol3::ExternalValidationMode::ProvidedOnly);
...
"#;
    ...
    options.policy.hash_algorithm_registry = sbol3::HashAlgorithmRegistry::Strict;
    let report = document.validate_with(options);
    assert!(
    ...
    let mut options = sbol3::ValidationOptions::default();
    options.policy.hash_algorithm_registry = sbol3::HashAlgorithmRegistry::Lenient;
    let report = document.validate_with(options);
    assert!(
        report
            .issues()
...
fn every_policy_blocked_rule_has_an_adr_file() {
    ...
    for status in sbol3::validation_rule_statuses() {
        if status.blocker == Some(sbol3::Blocker::Policy) {
...
fn applied_options_round_trips_overrides() {
    ...
    assert_eq!(summary.severity_floor, Some(sbol3::Severity::Warning));
...
"#;
    ...
    let mut options = sbol3::ValidationOptions::default();
    options.topology_completeness = sbol3::TopologyCompleteness::RequireKnownForNucleicAcids;
...
fn every_non_implemented_rule_has_blocker() {
    ...
    for status in sbol3::validation_rule_statuses() {
        ...
        let needs_blocker = !matches!(
            status.status,
            sbol3::RuleStatus::Error | sbol3::RuleStatus::Warning
        );
...
fn implemented_rule_severity_matches_status() {
    ...
    for status in sbol3::validation_rule_statuses() {
        let expected = match status.status {
            sbol3::RuleStatus::Error => Some(sbol3::NormativeSeverity::Must),
            sbol3::RuleStatus::Warning => Some(sbol3::NormativeSeverity::Should),
            _ => None,
        };
