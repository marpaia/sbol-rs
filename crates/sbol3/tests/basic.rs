...
fn targets_sbol_3_1_0() {
    assert_eq!(sbol3::SPEC_VERSION, "3.1.0");
    assert!(sbol3::SPECIFICATION_URL.ends_with("/version-3.1.0/"));
}
...
fn read_errors_report_rdf_parse_failures() {
    let error = sbol3::Document::read_turtle("@prefix broken").unwrap_err();
    assert!(error.to_string().contains("RDF parse error"));
}

#[test]
fn read_path_reports_missing_file_via_io_error() {
    let error = sbol3::Document::read_path("/tmp/sbol-rs-does-not-exist.ttl").unwrap_err();
    assert!(matches!(error, sbol3::ReadError::Io { .. }));
...
fn read_path_rejects_unknown_extension() {
    ...
    let error = sbol3::Document::read_path(&tmp).unwrap_err();
    let _ = std::fs::remove_file(&tmp);
    match error {
        sbol3::ReadError::UnknownFormat {
            extension: Some(ext),
            ..
        } => assert_eq!(ext, "xml"),
...
"#,
    ...
    for &format in sbol3::RdfFormat::ALL {
        ...
        let reparsed = sbol3::Document::read_path(&path)
...
"#,
    ...
    .unwrap();
    let object = document
        .get(&sbol3::Resource::iri("https://example.org/component"))
        .unwrap();

    use sbol3::ObjectClasses;

    assert!(object.has_class(sbol3::SbolClass::Component));
