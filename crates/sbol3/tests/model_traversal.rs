use sbol3::{Document, Resource, SbolClass, SbolObject};
...
fn sbol_class_iris_and_hierarchy_match_public_contract() {
    ...
    for (class, local_name, is_top_level) in classes {
        let iri = sbol3::Iri::new_unchecked(format!("http://sbols.org/v3#{local_name}"));
...
fn iris(values: &[sbol3::Iri]) -> Vec<&str> {
    values.iter().map(sbol3::Iri::as_str).collect()
}
