...
//! `cargo run -p sbol3 --bin generate-conformance-report` to refresh.
...
fn conformance_report_matches_committed_file() {
    ...
    assert_eq!(
        committed, rendered,
        "docs/conformance.md is stale. Run `cargo run -p sbol3 --bin generate-conformance-report` to refresh."
    );
}
