...
impl Document {
    /// Builds a document from owned typed SBOL 2 objects.
    ///
    /// The supplied typed values are retained as the document's primary typed
    /// surface; the RDF graph and property-bag indexed map are derived from
    /// them so the document is round-trip ready.
    pub fn from_objects(objects: Vec<Sbol2Object>) -> Result<Self, BuildError> {
