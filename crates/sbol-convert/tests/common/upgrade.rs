...
pub fn upgrade_fixture(name: &str) -> Document {
    ...
    let (document, report) = sbol_convert::upgrade_from_sbol2(&input, RdfFormat::Turtle)
...
pub fn upgrade_real(name: &str) -> (Document, sbol_convert::UpgradeReport) {
    let path = real_fixture(name);
    let input = std::fs::read_to_string(&path).unwrap();
    sbol_convert::upgrade_from_sbol2(&input, RdfFormat::RdfXml).expect("real fixture upgrade")
}
