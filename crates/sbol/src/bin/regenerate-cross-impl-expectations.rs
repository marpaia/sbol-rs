//! Regenerates `tests/fixtures/cross-impl/*.libSBOLj3.expected.{ttl,rdf,jsonld,nt}`
//! by running libSBOLj3 inside the pinned Docker image. For each
//! source fixture every output format is regenerated. Contributors
//! run this locally after a libSBOLj3 version bump; CI never invokes
//! it (CI only diffs the committed references via the `cross_impl`
//! integration test).
//!
//! Usage:
//!
...
const DOCKER_IMAGE: &str = "libsbolj3-pinned";

fn main() -> ExitCode {
    let workspace = workspace_root();
    let cross_impl_dir = workspace.join("tests/fixtures/cross-impl");
    ...
    for (stem, source) in FIXTURES {
        ...
        for &format in RdfFormat::ALL {
            let output_path =
                cross_impl_dir.join(format!("{stem}.libSBOLj3.expected.{}", format.extension()));
            match run_libsbolj3(&source_path, &output_path, format) {
                Ok(()) => {
...
fn libsbolj3_format_name(format: RdfFormat) -> &'static str {
    match format {
        RdfFormat::Turtle => "turtle",
        RdfFormat::RdfXml => "rdfxml",
        RdfFormat::JsonLd => "jsonld",
        RdfFormat::NTriples => "ntriples",
        _ => panic!("regenerate harness has no libSBOLj3 mapping for format {format}"),
    }
}
...
fn run_libsbolj3(source: &Path, output: &Path, format: RdfFormat) -> std::io::Result<()> {
    ...
    if !result.status.success() {
        return Err(std::io::Error::other(format!(
            "libSBOLj3 container exited with status {:?}",
            result.status
        )));
    }
