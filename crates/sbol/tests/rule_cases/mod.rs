...
pub fn read_case(case: RuleCase) -> sbol::Document {
    let turtle = format!("{PREFIXES}\n{}", case.body);
    sbol::Document::read_turtle(&turtle)
        .unwrap_or_else(|error| panic!("{} did not parse: {error}", case.name))
}

pub fn read_positive_case(case: PositiveCase) -> sbol::Document {
    let turtle = format!("{PREFIXES}\n{}", case.body);
    sbol::Document::read_turtle(&turtle)
        .unwrap_or_else(|error| panic!("{} did not parse: {error}", case.name))
}

pub fn assert_rule(report: &sbol::ValidationReport, rule: &str) {
    assert!(
...
pub fn assert_no_rule(report: &sbol::ValidationReport, rule: &str) {
    assert!(
