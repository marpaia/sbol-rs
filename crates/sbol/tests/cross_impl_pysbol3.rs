//! Cross-implementation conformance vs pySBOL3.
//!
//! For each `*.pySBOL3.expected.{ttl,rdf,jsonld,nt}` committed under
//! `tests/fixtures/cross-impl-pysbol3/`, parse the corresponding
//! SBOLTestSuite source fixture with sbol-rs, parse the pySBOL3
//! reference in its own format, and compare the two normalized triple
//! sets. The reference files are generated locally by running pySBOL3
//! in a pinned Docker image — see
//! `tests/fixtures/cross-impl-pysbol3/README.md` and
//! `crates/sbol/src/bin/regenerate-cross-impl-pysbol3-expectations.rs`.
//!
//! A divergence either gets fixed in sbol-rs or allowlisted with a
//! written rationale in
//! `tests/fixtures/cross-impl-pysbol3/allowlist.txt`.
...
/// `<stem>.pySBOL3.expected.<ext>` into its stem and `RdfFormat`.
/// Returns `None` for any file that doesn't fit the pattern (including
/// the harness's allowlist / Dockerfile / README sibling files).
fn parse_reference_name(reference_name: &str) -> Option<(&str, sbol::RdfFormat)> {
    let (stem_with_marker, ext) = reference_name.rsplit_once('.')?;
    let stem = stem_with_marker.strip_suffix(".pySBOL3.expected")?;
    let format = sbol::RdfFormat::from_extension(ext)?;
    Some((stem, format))
}

/// Maps a reference-file stem to the SBOLTestSuite source fixture path
/// it was generated from. Mirrors the libSBOLj3 harness's table so the
/// two harnesses can share fixtures one-to-one.
...
fn cross_impl_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("tests/fixtures/cross-impl-pysbol3")
}
...
#[test]
fn sbol_rs_output_matches_pysbol3_reference() {
    ...
    for entry in entries.flatten() {
        ...
        let Some(source) = source_fixture_for_stem(stem) else {
            panic!(
                "{name} has no source-fixture mapping in cross_impl_pysbol3.rs::source_fixture_for_stem; \
        ...
        let sbol_doc = sbol::Document::read_turtle(&source_text)
        ...
        let sbol_reparsed =
            sbol::Document::read(&sbol_serialized, format).unwrap_or_else(|error| {
                panic!("{source}: sbol-rs failed to reparse its own {format} output: {error}")
            });

        let reference_path = entry.path();
        let reference_doc = sbol::Document::read_path(&reference_path).unwrap_or_else(|error| {
            panic!("{name}: pySBOL3 {format} reference did not parse: {error}")
        });
        ...
        assert_eq!(
            ...
            "{source} produces a different normalized triple set than pySBOL3 in {format} \
             (reference: {name}). To allowlist a known-compliant divergence, add {name} to \
             tests/fixtures/cross-impl-pysbol3/allowlist.txt with a rationale."
        );
        compared += 1;
    }

    eprintln!(
        "cross-impl-pysbol3: compared {compared} fixture(s) against pySBOL3 reference output"
    );
}
