...
fn valid_fixtures_parse_and_validate() {
        ...
        .filter(|fixture| fixture.expectation == Expectation::Valid)
    {
        ...
        let document = sbol::Document::read_turtle(&contents)
...
"#,
    ...
    for (name, turtle, expected_rule) in cases {
        let document =
            sbol::Document::read_turtle(turtle).unwrap_or_else(|error| panic!("{name}: {error}"));
        let report = document.validate();
...
fn typed_views_expose_fixture_data() {
    ...
    let document = sbol::Document::read_turtle(&contents).unwrap();
...
fn representative_fixtures_round_trip_as_normalized_rdf() {
    ...
    for fixture_path in fixture_paths {
        ...
        for &format in sbol::RdfFormat::ALL {
            ...
            let reparsed = sbol::Document::read(&serialized, format).unwrap_or_else(|error| {
...
fn typed_round_trip_preserves_extension_triples_on_typed_objects() {
    ...
    let contents = fs::read_to_string(manifest_path(fixture_path)).unwrap();
    let document = sbol::Document::read_turtle(&contents).expect("fixture must parse");
    ...
    let rebuilt =
        sbol::Document::from_objects(vec![sbol::SbolObject::Component(component.clone())])
...
fn validation_rule_catalog_is_pinned_to_sbol_3_1_0() {
    assert_eq!(sbol::VALIDATION_RULE_SPEC_VERSION, sbol::SPEC_VERSION);
    assert_eq!(
        sbol::VALIDATION_RULE_SPEC_CANONICAL_URL,
        sbol::SPECIFICATION_URL
    );
    assert_eq!(sbol::VALIDATION_RULE_SPEC_PATH, "spec/SBOL3.1.0.md");
    assert_eq!(
        sbol::VALIDATION_RULE_SPEC_PDF_SHA256,
