...
//! [`sbol::downgrade`] — no external services or Docker required.

use std::env;
use std::time::Duration;

use sbol::{Document, RdfFormat, SbolIdentified, SbolTopLevel, UpgradeReport, UpgradeWarning};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let error_count = validation
        .issues()
        .iter()
        .filter(|i| matches!(i.severity, sbol::Severity::Error))
        .count();
    if error_count == 0 {
